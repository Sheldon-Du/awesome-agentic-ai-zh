...
> 📋 **Chapter Composition**: Why this stage exists → **Core mental model: the Types → Config → Repo → Service four-layer work boundary** (diagram) → 12 advanced concept skeletons → Why these 12 were chosen → **Core Harness Engineering Principles (multi-source, 4 categories + dependency map)** → Advanced agentic application flow (5 steps) → Complete reading path → Self-check
...
### 🔁 Failure-mode lifecycle (how F11-F14 evolved)

![Failure-Mode Evolution Cycle](../resources/diagrams/failure-lifecycle.en.png)

F11 / F12 / F13 / F14 are not random labels. They are [`agent-collab-skills` observed failure modes](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md), and each one goes through the same loop: **discover → document → encode as a check → eliminate automatically**.

```
              ① Incident discovered (dogfood / production fail)
                              │
                              ▼
              ② Documented in
                 ...
                 and named F-N
                              │
                              ▼
              ③ Added to an agent-acceptance-gate
                 preset YAML check (codified)
                              │
                              ▼
              ④ The next time the same situation appears,
                 it is automatically caught
                              │
                              │  Once enough accumulate →
                              ▼
              ⑤ Promoted into a mandatory invocation rule
                 in CLAUDE.md / SKILL.md
```

**Example: F14 (skipping a mandatory preset) through the full cycle**

```
  F14 incident (2026-05-13, Phase D operator skipped preset)
            │
            ▼
  observed-failure-modes.md §F14
  (named as a META-FAILURE)
            │
            ▼
  ...
  cross_document_link_text_parity check (v0.2.3)
            │
            ▼
  Retrospective test on commit 4a60742 catches 9+ real drifts
  (6 missed by audit, 3 already known to audit)
            │
            ▼
  CLAUDE.md Complex Task Protocol gains the rule:
  "if the trigger fires, the preset must run"
```

→ **This loop is the core evolution mechanism of `agent-collab-skills`**. The point is not to pre-write every rule. It is to **codify every failure once, and put the rule in YAML instead of keeping it in somebody's head**. Anthropic Skills `references/` and OpenAI Taste Invariants follow the same logic.
...
The diagram below groups the 12 concepts by **which stack layer they live in** (horizontal axis) and **what kind of problem they solve** (vertical axis), so you can see which concepts should be learned together and which you can skip for now.

```
                  Service layer     Repo layer      Config layer     Types layer
                ───────────────    ──────────────   ──────────────   ───────────────
 ┌ Orchestration ┐   1. Work Boundary*                                2. Contract
 │ how agents    │   3. Speculative                                     Hand-offs
 │ split work /  │   10. Self-organizing
 │ collaborate   │   Teams
 ├ Reflection ───┤   5. Plan-Act-Reflect   (memory provided          11. Spec-driven
 │ how agents    │   4. Agent-as-Judge    by Stage 6)                  Development
 │ self-correct  │
 ├ Governance ───┤   6. Hierarchical                 7. Autonomy
 │ autonomy /    │   Task Decomposition              Gradients
 │ boundary ctrl │                                  12. Graceful
 │               │                                  Degradation
 ├ Resilience ───┤                                  8. Cost-aware
 │ failure prev. │                                  Budget Gates
 │               │                                  9. Failure
 │               │                                  Injection
 └───────────────┘

  * Work Boundary spans all layers (discipline-level), not one specific stack position
```
...
Once you understand the 5 principles above plus the Anthropic cross-mapping, **how do you actually apply those ideas in agent design?** The diagram below answers that with a 5-step path from mental model to production.

```
                  Advanced Agentic Application Flow
              ═════════════════════════════════════

  Stage 7 complete (you can already build production agents)
                       │
                       ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 1  Build the mental model — four work      │
  │         boundary layers                         │
  │ ─────────────────────────────────────────────── │
  │ Types → Config → Repo → Service                 │
  │ Decide which stack layer the agent may touch    │
  │ and when crossing a layer is a violation        │
  │ 📍 This stage §🧭 Core mental model             │
  └─────────────────────┬───────────────────────────┘
                        │ Know what the agent may own
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 2  Pick 2-3 relevant advanced concepts     │
  │ ─────────────────────────────────────────────── │
  │ From the 12 skeletons, choose the concepts      │
  │ closest to your problem                         │
  │ (Work Boundary / Contract / PAR / Autonomy ...) │
  │ 📍 This stage §📚 12 advanced concepts          │
  └─────────────────────┬───────────────────────────┘
                        │ Know which patterns matter
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 3  Apply the 5 OpenAI principles           │
  │         (cross-cutting)                         │
  │ ─────────────────────────────────────────────── │
  │ Legibility / SoR / Progressive Disclosure /    │
  │ Taste Invariants / Throughput Merge Phil.       │
  │ These 5 principles cut across all 12 concepts   │
  │ and determine whether the design is "right"     │
  │ 📍 This stage §🔬 Core Harness Eng principles   │
  └─────────────────────┬───────────────────────────┘
                        │ Know what "doing it right" means
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 4  Encode into Skills + CLAUDE.md          │
  │ ─────────────────────────────────────────────── │
  │ Use the 4 prompts in Stage 5 to write:          │
  │ • CLAUDE.md audit / generate (Stage 5.1)        │
  │ • SKILL.md audit / generate (Stage 5.3)         │
  │ 📍 → [Stage 5.1](05-claude-code-ecosystem.en.md#51--claude-code-basics) + [Stage 5.3](05-claude-code-ecosystem.en.md#53--skillsclaude-codes-behavior-layer--the-most-critical-layer-of-the-claude-code-ecosystem) │
  └─────────────────────┬───────────────────────────┘
                        │ Turn the design into artifacts
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 5  Verify with an acceptance gate          │
  │ ─────────────────────────────────────────────── │
  │ • Preset YAML catches drift                     │
  │ • LLM-as-judge automates evaluation             │
  │ • Human spot-checks cover edge cases            │
  │ 📍 → [agent-collab-skills](https://github.com/WenyuChiou/agent-collab-skills) │
  └─────────────────────┬───────────────────────────┘
                        │
                        ▼
              Production agent ready
              (stable for real users, auto-verified, predictable failure modes)
```

→ **How to use this diagram**: the first time you read this stage, follow STEP 1 → 5 in order. Later, when an agent design gets stuck, come back and identify which step you are actually blocked on.

→ **Difference from the earlier Why → What → How diagram**: that one is a horizontal reference for mapping **pain points ↔ principles ↔ tools**. This one is a vertical execution path for **what to do after you finish learning**.
...
This is not just a reading list. It is a **decision tree**: identify the problem you have right now, then read the 1-2 papers or posts attached to that branch first.
...
```
        ...
        ┌───────────┬─────────┼─────────┬───────────┐
        ▼           ▼         ▼         ▼           ▼
   "I don't know  "Should I   "Context  "How do I   "I want to
    how to start   use multi-  feels     write evals keep up with
    with agents"   agent at    inefficient"or automate frontier
