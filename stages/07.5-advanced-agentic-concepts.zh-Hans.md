...
> 📋 **本章组成**：为什么有这 stage → **核心 mental model：Types → Config → Repo → Service 四层工作边界**（diagram）→ 12 个进阶概念 skeleton → 为什么选这 12 个 → **核心 Harness Engineering 原则（多 source、4 大类别 + 关系图）** → 进阶 agentic 应用流程图（5 step） → 完整 reading path → 自我检查
...
### 🔁 Failure-mode lifecycle（F11-F14 怎么进化的）

![Failure-Mode Evolution Cycle](../resources/diagrams/failure-lifecycle.zh-Hans.png)

F11 / F12 / F13 / F14 不是随机编号，而是 [`agent-collab-skills` 累积观察的 failure modes](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)。每一个都走过 **发现 → 文档化 → encode 成 check → 自动消除** 的循环：

```
              ① 发现 incident（dogfood / production fail）
                              │
                              ▼
              ② 在 docs/observed-failure-modes.md
                 文档化（命名为 F-N）
                              │
                              ▼
              ③ 加进 agent-acceptance-gate preset
                 YAML check（codify）
                              │
                              ▼
              ④ 下次同样情境自动被 catch
              ...
              ⑤ CLAUDE.md / SKILL.md 的
                 mandatory invocation 规则
```

**举例：F14（skipping mandatory preset）走完整循环**：

```
  F14 incident（2026-05-13、Phase D 操作者跳过 preset）
            │
            ▼
  observed-failure-modes.md §F14（命名为 META-FAILURE）
            │
            ▼
  ...
  cross_document_link_text_parity check（v0.2.3）
            │
            ▼
  retrospective test：对 commit 4a60742 跑、抓到 9+ real drift
  （6 个 audit 漏的、3 个 audit 已知的）
            │
            ▼
  CLAUDE.md Complex Task Protocol 写进
  「trigger 触发时 preset 必跑」规则
```

→ **这套循环是 agent-collab-skills 的核心进化机制**。重点不是“把所有规则一开始就写死”，而是“**每出一次包就 codify 一次，把规则写进 YAML，不写进脑袋**”。Anthropic Skills 的 `references/` 机制和 OpenAI 的 Taste Invariants，本质上是同一套逻辑。

## 📚 12 个进阶概念 — skeleton

每个概念控制在 4 行以内：一句话定义 + 属于 stack 哪一层 + 最值得读的 1 个资源。

### 🗺️ 12 概念 cluster map（stack 层 × 问题类型）

![12 Advanced Agentic AI Concepts — Cluster Map](../resources/diagrams/concept-cluster.zh-Hans.png)

下图把 12 个概念按 **“位于 stack 的哪一层”**（横轴）和 **“解决什么类型的问题”**（纵轴）分群，让你一眼看出哪些概念适合一起学、哪些可以暂时跳过：

```
                  Service 层         Repo 层         Config 层        Types 层
                ───────────────    ──────────────    ──────────────    ───────────────
 ┌─ 编排类 ──┐   1. Work Boundary*                                   2. Contract
 │  agent 怎  │   3. Speculative                                       Hand-offs
 │  分工 / 协  │   10. Self-organizing
 │  作        │   Teams
 ├─ 反思类 ──┤   5. Plan-Act-Reflect   (memory 由                  11. Spec-driven
 │  agent 怎  │   4. Agent-as-Judge    Stage 6 提供)                  Development
 │  自我修正  │
 ├─ 治理类 ──┤   6. Hierarchical                  7. Autonomy
 │  agent 自  │   Task Decomposition              Gradients
 │  主权边界  │                                  12. Graceful
 │            │                                  Degradation
 ├─ 韧性类 ──┤                                  8. Cost-aware
 │  agent 预  │                                  Budget Gates
 │  防失败    │                                  9. Failure
 │            │                                  Injection
 └────────────┘

  * Work Boundary 跨所有层（discipline 级），不是某个特定 stack 位置
```
...
## 🔬 核心 Harness Engineering 原则（多 source 整理）
      ...
      Memory Persistence ──extends to───→ Progressive Disclosure
        [Anth]            cross-session    [OAI + Anth]
              ...
              └────────────┬────────────────┘
                           ...
                           │ entry point
                           ▼
    ② Interface       Legibility ──让 agent 能 self-report──→ Transparency
                      [OAI]                                    [Anth]
                           │
...
### 📐 进阶 agentic 概念应用流程图（读者导引）

学完前面 5 条原则 + Anthropic 对照后，**怎么把这些概念真的用到 agent 设计上？** 下面这张流程图说明“**读完本 stage 之后该怎么动手**”——5 个步骤，从 mental model 一路走到 production：

```
                  进阶 Agentic 概念应用流程
              ═══════════════════════════════

  Stage 7 完成（你已经能做 production agent）
                       │
                       ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 1  建立 mental model — 四层工作边界         │
  │ ─────────────────────────────────────────────── │
  │ Types → Config → Repo → Service                 │
  │ 想清楚 agent 能动到 stack 哪一层、跨层算违规     │
  │ 📍 本 stage §🧭 核心 mental model               │
  └─────────────────────┬───────────────────────────┘
                        │ 知道 agent 该管什么
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 2  挑 2-3 个相关进阶概念                    │
  │ ─────────────────────────────────────────────── │
  │ 从 12 个 skeleton 找跟你问题最相关的            │
  ...
  └─────────────────────┬───────────────────────────┘
                        ...
                        ▼
  ┌─────────────────────────────────────────────────┐
  │ STEP 3  套 5 个 OpenAI 原则（cross-cutting）     │
  │ ─────────────────────────────────────────────── │
  │ Legibility / SoR / Progressive Disclosure /    │
  │ Taste Invariants / Throughput Merge Phil.       │
  │ 这 5 条原则跨越全部 12 个概念，决定“做得对不对”  │
  │ 📍 本 stage §🔬 核心 Harness Eng 原则           │
  └─────────────────────┬───────────────────────────┘
                        │ 知道怎样才算“做对”
                        ▼
  ┌─────────────────────────────────────────────────┐
  ...
  │ ─────────────────────────────────────────────── │
  │ 用 Stage 5 的 4 个 prompt 动手写：               │
  │ • CLAUDE.md audit / generate（Stage 5.1）        │
  │ • SKILL.md audit / generate（Stage 5.3）         │
  │ 📍 → [Stage 5.1](05-claude-code-ecosystem.zh-Hans.md#51--claude-code-基础) + [Stage 5.3](05-claude-code-ecosystem.zh-Hans.md#53--skillsclaude-code-的行为层-claude-code-生态最关键的一层) │
  └─────────────────────┬───────────────────────────┘
                        │ 设计真正落地
                        ▼
  ┌─────────────────────────────────────────────────┐
  ...
  │ • Preset YAML 抓 drift                           │
  │ • LLM-as-judge 自动评                            │
  │ • Human spot-check 补 edge case                  │
  │ 📍 → [agent-collab-skills](https://github.com/WenyuChiou/agent-collab-skills) │
  └─────────────────────┬───────────────────────────┘
                        │
                        ▼
              Production agent ready
              （可以稳定给人用、有自动验证、有可预测失败）
```

→ **怎么用这张图**：第一次读本 stage 时，按 STEP 1 → 5 顺序走；之后实作 agent 卡住时，再回来判断自己卡在了哪一步。

→ **它和前面 Why → What → How 图的区别**：前一张是“**痛点 ↔ 原则 ↔ 工具**”的横向对照，适合当 reference；这一张是“**从学完到上线**”的纵向流程，适合照着一步一步做。
...
这不是普通 reading list，而是 **decision tree**。你现在卡在哪个问题，就先读对应的那 1-2 篇：
...
```
        ...
        ┌───────────┬─────────┼─────────┬───────────┐
        ▼           ▼         ▼         ▼           ▼
   「不知道     「multi-     「context  「eval 怎么   「想跟上
    agent       agent 要     没效率」   写 / 自动    frontier
    怎么开      不要用、               验证」       现况」
    始」        怎么开」
        │           │         │         │           │
        ▼           ▼         ▼         ▼           ▼
...
→ **规则**：每个分支**最多挑 2 篇深读**。读完再回来看自己还有什么问题，再决定下一篇，不要 broad-scan 整个清单。
